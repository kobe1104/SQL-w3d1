...
def cl_to_lr
  ...
  -- JOIN
  --   stops a_stop ON a.stop_id = a_stop.id
  -- JOIN
  --   stops b_stop ON b.stop_id = b_stop.id
  -- WHERE
  --   a_stop.name = 'Craiglockhart' AND b_stop.name = 'London Road'
...
def craiglockhart_to_sighthill
  ...
  execute(<<-SQL)
    SELECT DISTINCT
      a.num, a.company, stopd.name, d.num, d.company
    FROM
      routes a
    JOIN
      routes b ON (a.company = b.company AND a.num = b.num)
    JOIN
      routes d ON (d.stop_id = b.stop_id)
    JOIN
      routes e ON (e.company = d.company AND e.num = d.num)
    ...
    WHERE
      stopa.name = 'Craiglockhart' AND stope.name = 'Sighthill'
  SQL
end
